  ...
  return (
    <span className="flex gap-0.5 text-[10px] leading-none" title={`Autonomy level ${level}/5`}>
      {Array.from({ length: 5 }, (_, i) => (
        <span
          key={i}
          className={i < level ? "text-green-500" : "text-zinc-300 dark:text-zinc-600"}
        >
          ●
        </span>
      ))}
...
export function UserTaskNode({ data, selected }: NodeProps & { data: BPMNNodeData }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 border-b border-zinc-100 px-3 py-2 dark:border-zinc-800">
        <User className="h-4 w-4 shrink-0 text-green-500 dark:text-green-400" />
      ...
      <Handle
        ...
        className="!h-3 !w-3 !border-2 !border-green-400 !bg-white dark:!bg-zinc-900"
      />
      <Handle
        type="source"
